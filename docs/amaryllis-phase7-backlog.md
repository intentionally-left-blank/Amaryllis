...
| ID | Status | Task | Deliverable | Definition of Done |
|---|---|---|---|---|
| P7-D01 | todo | Add digest artifact + inbox delivery format | `api/inbox_api.py` integration + schema | Digest lands in inbox with summary, key links, and action hints |
| P7-D02 | todo | Add optional outbound channels (webhook/email/telegram adapter) | delivery adapters + policy controls | User can opt in to external delivery channels with per-channel limits |

### Epic E - Reliability and Security Gates

| ID | Status | Task | Deliverable | Definition of Done |
|---|---|---|---|---|
| P7-E01 | todo | Add release gate for news mission E2E flow | `scripts/release/news_mission_gate.py` + CI wiring | CI fails if schedule, ingestion, dedup, or digest citation contracts regress |
| P7-E02 | todo | Add security gate for provider session handling | `scripts/security/provider_session_policy_check.py` | CI fails on token leakage, missing revocation checks, or missing auth boundaries |
| P7-E03 | todo | Add replay fixtures for deterministic digest output | `eval/fixtures/replay/news/*` + tests | Same input snapshot produces stable digest structure and provenance fields |
...
| P7-C04 | todo | `ai_news_daily` automation template |
| P7-D01 | todo | Inbox digest delivery UX contract |
| P7-E01 | todo | News mission E2E release gate |
| P7-E02 | todo | Provider session security gate |
| P7-E03 | todo | Replay determinism fixtures |
