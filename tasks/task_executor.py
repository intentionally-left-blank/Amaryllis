...
class TaskExecutor:
    ...
    def _execute_step_fetch_source(self, ctx: StepExecutionContext) -> StepExecutionResult:
        ...
        for url in source_urls[:5]:
            fetched = self._fetch_source_url(url)
            ...
            if text:
                ...
                for sentence in self._extract_fact_candidates(text, max_items=4):
    ...
    def _execute_step_extract_facts(self, ctx: StepExecutionContext) -> StepExecutionResult:
        payload = self._base_step_payload(ctx)
        evidence_items = self._collect_dependency_evidence_items(ctx.dependency_artifacts)
        extracted: list[str] = []
        ...
        for source in evidence_items:
            ...
            if not text:
                continue
            for candidate in self._extract_fact_candidates(text, max_items=8):
    ...
    def _execute_step_verify_like(self, ctx: StepExecutionContext) -> StepExecutionResult:
        ...
        claims = self._collect_claims_for_verification(ctx.dependency_artifacts, ctx.dependency_points)
        evidence_items = self._collect_dependency_evidence_items(
        ...
        for claim in claims[:14]:
            match = self._best_claim_support(claim=claim, evidence_items=evidence_items)
    ...
    def _fetch_source_url(self, url: str) -> dict[str, Any]:
        ...
        try:
            ...
            clean_text = self._sanitize_html_to_text(raw_text)
    ...
    def _best_claim_support(
        ...
        *,
        ...
        claim_tokens = self._tokenize_overlap_tokens(claim)
        if not claim_tokens:
        ...
        for item in evidence_items:
            ...
            if not text:
                continue
            text_tokens = self._tokenize_overlap_tokens(text)
            if not text_tokens:
                continue
